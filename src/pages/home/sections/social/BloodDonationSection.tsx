...
import { Calendar, MapPin, ChevronDown, ChevronUp, ChevronLeft, ChevronRight, X } from "lucide-react";
...
const BloodDonationSection = ({ lang }: Props) => {
  // Fix: Explicitly defining types to avoid TS error
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <div className="lg:col-span-7">
                  <div className="grid grid-cols-4 gap-4">
                    <div className="col-span-4 h-64 md:h-96 rounded-[2.5rem] overflow-hidden cursor-pointer shadow-xl" 
                    ...
                    {event.images.slice(0, 3).map((img, i) => (
                      <div key={i} className={`h-24 md:h-36 rounded-2xl overflow-hidden cursor-pointer hover:scale-105 transition-transform ${i === 0 ? 'col-span-2' : 'col-span-1'}`}
                      ...
                      <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 pt-4">
                          {event.images.slice(3).map((img, idx) => (
                            <div key={idx} className="aspect-video rounded-2xl overflow-hidden cursor-pointer shadow-md hover:scale-105 transition-transform"
                                 onClick={() => setSliderData({ images: allImages, index: idx + 4 })}>
                              <img src={img} className="w-full h-full object-cover" alt="gallery-extra" />
                            </div>
                          ))}
                        </div>
                      </motion.div>
        ...
        {sliderData && (
          ...
          >
            ...
            <div className="relative w-full max-w-6xl flex items-center justify-center">
              <button onClick={prevImg} className="absolute -left-4 md:-left-20 p-5 bg-black/5 hover:bg-black/10 rounded-full z-50"><ChevronLeft className="w-8 h-8" /></button>
              <motion.img key={sliderData.index} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} src={sliderData.images[sliderData.index]} className="max-h-[75vh] w-full object-contain rounded-[3rem] shadow-2xl" />
              <button onClick={nextImg} className="absolute -right-4 md:-right-20 p-5 bg-black/5 hover:bg-black/10 rounded-full z-50"><ChevronRight className="w-8 h-8" /></button>
            </div>
            <button onClick={() => setSliderData(null)} className="mt-12 px-12 py-5 bg-black text-white rounded-full font-black flex items-center gap-3 hover:scale-105 transition-all shadow-xl">
