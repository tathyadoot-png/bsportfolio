export const sammanData = {
  title: {
    hi: "सम्मान जो समाज के काम आए",
    en: "Awards for Social Good"
  },

  tag: "Inspiration",

  description: {
    hi: "यह एक अनूठी पहल है, जिसमें प्राप्त सम्मान और पुरस्कारों को समाज के हित में उपयोग किया जाता है, ताकि जरूरतमंद लोगों की सहायता की जा सके। यह प्रयास समाज में सेवा और समर्पण की भावना को मजबूत करता है।",
    en: "A unique initiative where received awards and honors are utilized for social welfare, helping those in need. This effort strengthens the spirit of service and contribution in society."
  },

  highlights: [
    {
      title: {
        hi: "सम्मान का सदुपयोग",
        en: "Meaningful Use of Awards"
      },
      desc: {
        hi: "प्राप्त सम्मान को समाज सेवा और जरूरतमंदों की सहायता में उपयोग किया जाता है।",
        en: "Awards are utilized for social service and helping the needy."
      }
    },
    {
      title: {
        hi: "समाज हित में योगदान",
        en: "Contribution to Society"
      },
      desc: {
        hi: "समाज के कमजोर वर्गों को सहयोग और समर्थन प्रदान किया जाता है।",
        en: "Support is provided to underprivileged sections of society."
      }
    },
    {
      title: {
        hi: "प्रेरणा का स्रोत",
        en: "Source of Inspiration"
      },
      desc: {
        hi: "यह पहल दूसरों को भी समाज सेवा के लिए प्रेरित करती है।",
        en: "This initiative inspires others to contribute to society."
      }
    }
  ]
};